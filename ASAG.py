...
from sklearn.metrics import roc_curve, auc, hamming_loss, accuracy_score, cohen_kappa_score, confusion_matrix
...
def cohens_kappa(y_pred: Tensor, y_true: Tensor):
    ...
    def __init__(
        ...
        if test_file:
            ...
            test_examples = processor.get_dev_examples(
                    test_file, text_col=text_col, label_col=label_col
                )
...
class Learner(object):
    ...
    def save_model(self, path_ext):

        path = self.output_dir / path_ext
...
def load_model(dataBunch, pretrained_path, finetuned_wgts_path, device,
    ...
    if finetuned_wgts_path:
        ...
        #    finetuned_wgts_path, config=config)
        model = model_class[1].from_pretrained(pretrained_model_name_or_path=None,
                                               config=config,
    ...
    def fit(
        ...
        for epoch in pbar:
            ...
            for step, batch in enumerate(progress_bar(train_dataloader, parent=pbar)):
                ...
                output_list.append((batch[0], batch[3], outputs))
...
class BertClassificationPredictor(object):
    ...
    def get_learner(self):
        databunch = BertDataBunchCLS(
            ...
            val_file=None,
            test_data=None,
            label_file=self.construct + '_labels.csv',
            batch_size_per_gpu=16,
            max_seq_length=512,
            multi_gpu=False,
